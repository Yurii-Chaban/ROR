...
def maxMinString(numbers)
	...
	numbers.map do |num|
		...
		stringDigit = stringDigit + num.split('').min
		stringDigit = stringDigit + num.split('').max
...
puts "#1 Generate a hash, key will be an element itself, value will be a max digit in a element"
puts "\n"
...
puts "#2 Generate a string, string will contain a maximum and minimum digit of each element"
puts "\n"
...
puts "#3 Generate a number, number will be a summa of two maximum and two minimum elements"
puts "\n"
puts summa(numbers)
puts "\n"